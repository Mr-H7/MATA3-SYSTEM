      ...
      <section className="pos-catalogue">
        ...
        <div className="p-5 lg:p-6">
          ...
          {loading ? <div className="empty-state">Loading catalogue…</div> : <div className="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
            {filteredProducts.map((row) => { const available = stock(row); const depleted = available !== null && available <= 0; return <article className={`product-tile${depleted ? " opacity-60" : ""}`} key={row.id}><div className="flex items-start justify-between gap-3"><div><div className="font-mono text-[10px] text-[#6e685e]">{row.variant?.sku || row.product.sku}</div><h2 className="mt-2 text-base font-bold">{row.product.nameEn}</h2><p className="text-xs text-[#6e685e]">{[row.product.brand, row.product.model].filter(Boolean).join(" · ") || row.product.nameAr}</p></div>{available === null ? <span className="badge">{row.inventoryType.replaceAll("_", " ")}</span> : <span className={available <= 0 ? "badge badge-danger" : available <= 3 ? "badge badge-warning" : "badge badge-success"}>{available <= 0 ? "Out of stock" : `${available} in stock`}</span>}</div>{row.variant ? <div className="mt-4"><div className="eyebrow">Variant</div><span className="badge mt-1">{[row.variant.color, row.variant.size].filter(Boolean).join(" / ") || row.variant.sku}</span></div> : <div className="h-12" />}<div className="mt-4 flex items-end justify-between border-t border-[#e5ded3] pt-3"><div><div className="text-xs text-[#6e685e]">Selling price</div><strong className="text-lg">{Number(row.price).toFixed(2)} <span className="text-xs text-[#8c7356]">{row.market.currency}</span></strong></div><button className="btn btn-primary" disabled={depleted} onClick={() => addProduct(row)}>＋ Add</button></div></article>; })}
