...
type ProductRow = {
  id: string; price: string | number; availableStock?: number; inventoryType: string;
  market: { code: MarketCode; currency: string };
  product: { nameEn: string; nameAr: string; sku: string; brand?: string | null; model?: string | null; category?: { name: string } | null; primaryImageUrl?: string | null };
...
type CartItem = { key: string; listingId?: string; bundleId?: string; name: string; detail: string; price: number; quantity: number; imageUrl?: string | null };
...
export default function NewSale() {
  ...
  const stock = (row: ProductRow) => row.inventoryType === "PHYSICAL_STOCK" ? row.availableStock ?? Math.max(0, (row.inventory?.currentStock ?? 0) - (row.inventory?.reservedStock ?? 0)) : null;
  ...
  const addProduct = (row: ProductRow) => addItem({ key: `listing-${row.id}`, listingId: row.id, name: row.product.nameEn, detail: row.variant ? [row.variant.sku, row.variant.color, row.variant.size].filter(Boolean).join(" · ") : row.product.sku, price: Number(row.price), imageUrl: row.product.primaryImageUrl });
      ...
      <section className="pos-catalogue">
        ...
        <div className="p-5 lg:p-6">
          ...
          {loading ? <div className="empty-state">Loading catalogue…</div> : <div className="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
            {filteredProducts.map((row) => { const available = stock(row); const depleted = available !== null && available <= 0; return <article className={`product-tile${depleted ? " opacity-60" : ""}`} key={row.id}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3">{row.product.primaryImageUrl ? <Image unoptimized src={row.product.primaryImageUrl} alt="" width={56} height={56} className="h-14 w-14 shrink-0 rounded object-cover" /> : <span className="inline-flex h-14 w-14 shrink-0 items-center justify-center rounded bg-[#f7eee7] text-xs text-[#8c7356]">No image</span>}<div className="min-w-0"><div className="font-mono text-[10px] text-[#6e685e]">{row.variant?.sku || row.product.sku}</div><h2 className="mt-2 truncate text-base font-bold">{row.product.nameEn}</h2><p className="truncate text-xs text-[#6e685e]">{[row.product.brand, row.product.model].filter(Boolean).join(" · ") || row.product.nameAr}</p></div></div>{available === null ? <span className="badge">{row.inventoryType.replaceAll("_", " ")}</span> : <span className={available <= 0 ? "badge badge-danger" : available <= 3 ? "badge badge-warning" : "badge badge-success"}>{available <= 0 ? "Out of stock" : `${available} in stock`}</span>}</div>{row.variant ? <div className="mt-4"><div className="eyebrow">Variant</div><span className="badge mt-1">{[row.variant.color, row.variant.size].filter(Boolean).join(" / ") || row.variant.sku}</span></div> : <div className="h-12" />}<div className="mt-4 flex items-end justify-between border-t border-[#e5ded3] pt-3"><div><div className="text-xs text-[#6e685e]">Selling price</div><strong className="text-lg">{Number(row.price).toFixed(2)} <span className="text-xs text-[#8c7356]">{row.market.currency}</span></strong></div><button className="btn btn-primary" disabled={depleted} onClick={() => addProduct(row)}>＋ Add</button></div></article>; })}
