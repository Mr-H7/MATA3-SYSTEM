...
export async function GET(request: NextRequest) {
  ...
  if (isSeller(user)) {
    const rows = await prisma.marketListing.findMany({ where, select: { id: true, price: true, available: true, inventoryType: true, market: { select: { code: true, currency: true } }, product: { select: { id: true, sku: true, nameAr: true, nameEn: true, brand: true, model: true, category: { select: { name: true } } } }, variant: { select: { id: true, sku: true, color: true, size: true } }, inventory: { select: { currentStock: true, reservedStock: true } } }, orderBy: { product: { nameEn: "asc" } } });
