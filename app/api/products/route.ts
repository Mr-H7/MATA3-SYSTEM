...
export async function POST(request: Request) {
  try {
    ...
    if (listings.some((listing) => Number(listing.cost) < 0 || Number(listing.price) < 0 || !Number.isInteger(Number(listing.minimumStock ?? 0)) || Number(listing.minimumStock ?? 0) < 0)) {
      return NextResponse.json({ error: "Costs, prices, and minimum stock must be non-negative" }, { status: 400 });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const baseSlug = slugify(body.publicSlug || body.nameEn);
      let publicSlug = baseSlug || null;
      if (publicSlug) {
        const conflict = await tx.product.findFirst({ where: { publicSlug } });
        if (conflict) publicSlug = `${publicSlug}-${body.sku.trim().toLowerCase()}`;
      }
      ...
      const product = await tx.product.create({
        data: {
          ...
          gender: (body.gender || Gender.NA) as Gender,
          status: (body.status || ProductStatus.DRAFT) as ProductStatus,
      ...
      for (const variant of variants) {
        ...
        createdVariants.set(created.sku, created.id);
      }
      for (const listingInput of listings) {
        ...
        const sourceMarketId = await inventorySourceMarketId(tx, market.id, listingInput.inventorySourceMarketCode || market.code);
        const listing = await tx.marketListing.create({
          data: {
            productId: product.id,
            variantId: variantId ?? null,
            marketId: market.id,
            supplierId: listingInput.supplierId || null,
            cost: new Prisma.Decimal(listingInput.cost),
            price: new Prisma.Decimal(listingInput.price),
            compareAt: listingInput.compareAt == null ? null : new Prisma.Decimal(listingInput.compareAt),
            available: listingInput.available ?? true,
            inventoryType: listingInput.inventoryType || InventoryType.PHYSICAL_STOCK,
            minimumStock: Number(listingInput.minimumStock ?? 0),
            notes: listingInput.notes || null,
        ...
        await tx.inventory.create({
          data: {
            ...
            marketId: market.id,
            currentStock: listing.inventoryType === InventoryType.PHYSICAL_STOCK ? Number(listingInput.stock ?? 0) : 0,
          },
        });
      }
